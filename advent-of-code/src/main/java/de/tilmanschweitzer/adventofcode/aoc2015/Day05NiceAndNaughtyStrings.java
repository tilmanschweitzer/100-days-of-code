...
public class Day05NiceAndNaughtyStrings extends MultiLineAdventOfCodeDay<String, Long> {

    public Day05NiceAndNaughtyStrings() {
        super(2015, 5);
    }
    ...
    public Long getResultOfFirstPuzzle(final List<String> input) {
        return input.stream().filter(Day05NiceAndNaughtyStrings::isNiceString).count();
    }

    @Override
    public Long getResultOfSecondPuzzle(final List<String> input) {
        return input.stream().filter(Day05NiceAndNaughtyStrings::isNiceStringVersion2).count();
    }
